...
- `person`: `{ id, name, position, department, email, role, type, selfScores, mgrScores, deduction, disciplined, tasks335[], leadScores{d,dd,e}, digital, selfNote, mgrNote, trackings[], approved, approvedBy, approvedRole, approvedAt }`. `tasks335[]` = `{catalogId,objId,assigned,completed,qualityIssues,delays,note}`; `leadScores` = d/đ/e của lãnh đạo (100/50). *(Dữ liệu cũ có thể còn `proTasks`/`hdScores` từ bản PRO — đã gỡ, nay bỏ qua.)*
- **Phê duyệt** (`approved/approvedBy/approvedRole/approvedAt`): cấp có thẩm quyền (`mgrEditable`) bấm "Phê duyệt & xuất phiếu" → đóng dấu (tên + vai trò + ngày dd/mm/yyyy). Lưu theo kỳ; reset khi sang kỳ mới/đặt lại. `upCur` tự GỠ phê duyệt khi sửa trường chấm điểm (`SCORING_KEYS` = selfScores/mgrScores/tasks335/leadScores/deduction/disciplined).
- **Điểm Nhóm I kẹp khi ĐỌC** theo `it.max` (computePerson + tab Đánh giá + xuất Word): dữ liệu cũ lưu trước khi đổi bộ tiêu chí (id giữ nguyên nhưng max giảm) không còn hiển thị/tính vượt trần mới.
