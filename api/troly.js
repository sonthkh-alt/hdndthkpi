...
export default async function handler(req, res) {
  ...
  // Kiểm tra yêu cầu HỢP LỆ trước — kẻo trừ lượt xong mới báo lỗi thì mất lượt oan.
  const body = typeof req.body === 'string' ? safeJson(req.body) : (req.body || {});
  const viec = String(body.viec || '');
  const turns = viec === 'hoidap'
    ? (Array.isArray(body.turns) ? body.turns : [])
        .slice(-MAX_TURNS)
        .map((t) => ({ role: t.role === 'assistant' ? 'assistant' : 'user', text: catNguCanh(t.text, MAX_TURN_CHARS) }))
        .filter((t) => t.text)
    : [];
  if (viec === 'hoidap' && !turns.length) return res.status(400).json({ error: 'Chưa có câu hỏi.' });
  if (viec !== 'hoidap' && !VIEC[viec]) return res.status(400).json({ error: `Không có việc "${viec}".` });

  // Có thẻ đăng nhập hợp lệ thì tính theo tài khoản, không thì tính là khách.
  const user = await verifyUser(req);

  // Trừ lượt TRƯỚC khi gọi AI để bấm liên tiếp không lọt trần chi phí.
