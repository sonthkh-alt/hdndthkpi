...
import {
  CRITERIA, classify, gradeClass, statusOf, clamp, task335Score, getND335Groups, computePerson, setCatalogRegistry, findCatalogItem, taskBreakdown,
                ...
                <section className={`${card} p-4 space-y-3`}>
                  <div className="rounded-xl border border-white/10 bg-white/5 p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center">
                      {[['Đạt đủ SL', `${(curC.st?.doneRate ?? 100).toFixed(0)}%`, 'HTXS 100%', (curC.st?.n ? curC.st.doneRate >= 100 : null)], ['Vượt mức', `${(curC.st?.exceedRate ?? 0).toFixed(0)}%`, 'HTXS ≥30%', (curC.st?.n ? curC.st.exceedRate >= 30 : null)], ['Không HT', `${(curC.st?.failRate ?? 0).toFixed(0)}%`, curC.leader ? 'lãnh đạo ≤30%' : '≤50%', (curC.st?.n ? curC.st.failRate <= (curC.leader ? 30 : 50) : null)]].map(([l, v, h, ok]) => (
                    ...
                    {(() => { const { failed: fl, partial: pa, delayed: dl } = taskBreakdown(cur.tasks335); const tn = (cur.tasks335 || []).filter((t) => t.catalogId).length; const nm = (t) => { const it = findCatalogItem(t.catalogId); return (it && it.name) || t.note || `[${t.catalogId}]`; }; return (<>
                      {fl.length > 0 && <div className="text-[11px] text-rose-200/90"><b className="text-rose-300">Không hoàn thành — dưới 50% SL ({fl.length}/{tn}):</b><ul className="list-disc pl-4 mt-0.5 space-y-0.5">{fl.map((t, i) => <li key={i}>{nm(t)} — {Number(t.completed) || 0}/{Number(t.assigned) || 0}{t.note ? ` · ${t.note}` : ''}</li>)}</ul></div>}
                      {pa.length > 0 && <div className="text-[11px] text-amber-200/90"><b className="text-amber-300">Chưa đạt đủ số lượng ({pa.length}/{tn}):</b><ul className="list-disc pl-4 mt-0.5 space-y-0.5">{pa.map((t, i) => <li key={i}>{nm(t)} — {Number(t.completed) || 0}/{Number(t.assigned) || 0}{t.note ? ` · ${t.note}` : ''}</li>)}</ul></div>}
                      {dl.length > 0 && <div className="text-[11px] text-orange-200/90"><b className="text-orange-300">Chậm tiến độ ({dl.length}/{tn}):</b><ul className="list-disc pl-4 mt-0.5 space-y-0.5">{dl.map((t, i) => <li key={i}>{nm(t)} — chậm {Number(t.delays) || 0} lần{t.note ? ` · ${t.note}` : ''}</li>)}</ul></div>}
                    </>); })()}
