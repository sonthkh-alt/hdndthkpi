              ...
              <div className="space-y-5">
                ...
                <section className={`${card} p-4 space-y-3`}>
                  <div className="rounded-xl border border-white/10 bg-white/5 p-3 space-y-2">
                    ...
                    {(() => { const tl = (cur.tasks335 || []).filter((t) => t.catalogId); const nm = (t) => { const it = findCatalogItem(t.catalogId); return (it && it.name) || t.note || `[${t.catalogId}]`; }; const r = (t) => { const as = Number(t.assigned) || 0, cp = Number(t.completed) || 0; return as > 0 ? cp / as : 0; }; const fl = tl.filter((t) => r(t) < 0.5); const pa = tl.filter((t) => r(t) >= 0.5 && r(t) < 1); const dl = tl.filter((t) => (Number(t.delays) || 0) > 0); return (<>
                      {fl.length > 0 && <div className="text-[11px] text-rose-200/90"><b className="text-rose-300">Không hoàn thành — dưới 50% SL ({fl.length}/{tl.length}):</b><ul className="list-disc pl-4 mt-0.5 space-y-0.5">{fl.map((t, i) => <li key={i}>{nm(t)} — {Number(t.completed) || 0}/{Number(t.assigned) || 0}{t.note ? ` · ${t.note}` : ''}</li>)}</ul></div>}
                      {pa.length > 0 && <div className="text-[11px] text-amber-200/90"><b className="text-amber-300">Chưa đạt đủ số lượng ({pa.length}/{tl.length}):</b><ul className="list-disc pl-4 mt-0.5 space-y-0.5">{pa.map((t, i) => <li key={i}>{nm(t)} — {Number(t.completed) || 0}/{Number(t.assigned) || 0}{t.note ? ` · ${t.note}` : ''}</li>)}</ul></div>}
                      {dl.length > 0 && <div className="text-[11px] text-orange-200/90"><b className="text-orange-300">Chậm tiến độ ({dl.length}/{tl.length}):</b><ul className="list-disc pl-4 mt-0.5 space-y-0.5">{dl.map((t, i) => <li key={i}>{nm(t)} — chậm {Number(t.delays) || 0} lần{t.note ? ` · ${t.note}` : ''}</li>)}</ul></div>}
                    </>); })()}
