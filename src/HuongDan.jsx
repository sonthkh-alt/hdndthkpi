...
import {
  ...
  Phone, Mail, Send, CheckCircle2, AlertTriangle, KeyRound, CalendarDays, ScanSearch, Sparkles, Database, Vote, HelpCircle, ArrowRight, Info,
} from 'lucide-react';
...
const ICONS = { Target, ShieldCheck, Landmark, Users, BookOpen, CalendarDays, ScanSearch, Sparkles, Database, Vote };
...
const FAQ = [
  ...
  { q: 'Phân hệ Trợ lý AI dùng thế nào, có tốn phí không?', a: 'Mở Trang chủ → thẻ “Trợ lý AI nghiệp vụ dân cử”. Mỗi lượt bấm tạo nội dung là một lượt gọi dịch vụ AI có tính phí theo khóa của cơ quan, nên có hạn mức theo ngày: khách chưa đăng nhập được dùng thử 1 lượt/ngày, người đăng nhập bằng tài khoản cơ quan được 5 lượt/ngày (số lượt còn lại hiện ngay đầu phân hệ, đặt lại lúc 0 giờ). Tài liệu tải lên để trích chữ không tính vào hạn mức, tối đa 3 MB mỗi tệp; tệp PDF bản chụp (scan) không trích được chữ. Kết quả do AI sinh ra chỉ để tham khảo — phải rà soát thể thức, căn cứ pháp lý và số liệu trước khi trình ký.' },
              ...
              {MODULES.filter((m) => m.id !== 'guide').map((m) => {
                ...
                return (
                  <button key={m.id} onClick={() => open(m.route)} className="text-left rounded-2xl border border-slate-200 bg-white p-4 hover:border-slate-300 hover:shadow-sm transition">
                    ...
                    {m.target?.kind === 'troly' && (
                      <p className="text-[11px] text-cyan-700 mt-1">Gọi AI có hạn mức theo ngày (khách 1 lượt · đăng nhập 5 lượt); nội dung do AI sinh ra phải được rà soát trước khi sử dụng.</p>
                    )}
                  </button>
                );
              })}
